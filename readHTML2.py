...
def getText(file):
    ...
    '''
    findCelsius = soup.get_text()
    
    for c, nextword in itertools.izip_longest(celsius, celsius[:1]):     
        if nextword.islower:
            c = c.replace(" C.", "Celsius")
        if nextword.isupper:
            c = c.replace(" C.", "Celsius.")
    '''
    '''
    celsius = 
    celsius = [tok for tok in soup.find_all("div", text = " C.")]
    '''
    '''
    for celsius in soup.find_all('div', text = " C. "):
        celsius = celsius.string.replace(" C. ", " CELSIUS ")
        wordaftercelsius = celsius.nextObject
        if wordaftercelsius.islower():
            #print("CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC")
            #celsius = celsius.replace(" C."," Celsius")
            celsius.replace_with(" Celsius")
        if wordaftercelsius.isupper():
            #celsius = celsius.replace(" C."," Celsius.")
            celsius.replace_with(" Celsius.")
    '''
    ...
    '''
    #soup = soup.replace(' C.', 'Celsius')
    for celsius in soup.find_all(u" C."): 
        wordaftercelsius = celsius.next_element
        
        if wordaftercelsius.str.islower:
            celsius = soup.replace(u' C.', u" Celsius")
        else:
            soup = soup.replace(u' C.', u" Celsius.")
    '''
    ''
    ...
    for claim in soup.find_all("div", class_ = "claim-text"):
        claim.get_text()    
    ...
    text = abstract.get_text() + claim.get_text() + description.get_text()
    ...
    for IFI in soup.find('span', text = 'Data provided by IFI CLAIMS Patent Services'):
        IFI.extract_jree()           #removing txt: "Data provided by IFI Claims patent services"
    ...
    for t in text:
        if unicode(t) == " C. ": # and text.next().islower():
            if t.next_element.islower():
                text = text.replace(" C.", " Celsius")
            if t.next_element.isupper():
                text = text.replace(" C.", " Celsius.")
    '''
