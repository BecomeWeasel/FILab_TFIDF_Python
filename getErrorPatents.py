...
def getpatents(targetlist):
    ...
    csvfile_ouput_by_year = open(getpatents_directory_output + 'ERRORPATENTS.csv', 'w+')
    ...
    # print(PATNUM[206])
    while normalcount < len(targetlist):
        PAT_HEADER = targetlist[normalcount]
        # print(PAT_HEADER)

        url = ''.join(['https://patents.google.com/patent/', PATNUM[PAT_HEADER - 1],
                       'A/en'])  # row 1 in reader_PatNO is stored PATNUM[0].
    ...
    timeend = time.time()
    print("it takes {0} sec for the get Patent text of {1}".format((timeend - timestart), len(targetlist)))

    return None


getpatents([450,1147,2404,2896,2940,4001,4098,4475])
